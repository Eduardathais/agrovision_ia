...
def list_events(limit: int = 50) -> list[dict]:
    ...
    conn.close()
    return [
        {"id": r[0], "event_time": r[1], "label": r[2], "confidence": r[3], "image_path": r[4]}
        for r in rows
    ]
...
def get_last_event() -> dict | None:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, event_time, label, confidence, image_path FROM events ORDER BY event_time DESC LIMIT 1"
    )
    row = cur.fetchone()
    conn.close()
